...
export default class MarketsPage extends BasePage {
  ...
  setHighLow () {
    ...
    const cache = mkt?.candleCaches[fiveMinBinKey]
    if (!cache) {
      for (const s of this.stats) {
...
class OrderTableRowManager {
  ...
  constructor (tableRow: HTMLElement, orderBin: MiniOrder[], baseUnitInfo: UnitInfo, quoteUnitInfo: UnitInfo, rateStep: number) {
    ...
    this.rateText = Doc.formatRateToRateStep(this.msgRate, baseUnitInfo, quoteUnitInfo, rateStep)
    this.setRateEl()
    this.setEpochEl()
  ...
  setRateEl () {
    ...
    if (this.msgRate === 0) {
      ...
      rateEl.innerText = this.rateText
