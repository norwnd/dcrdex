...
func (m *basicMarketMaker) orderPrice(basisPrice, bestPrice, feeAdj uint64, sell bool, gapFactor float64) uint64 {
	if m.cfg().GapStrategy == GapStrategyCompetitive {
		m.log.Tracef(
			"prepare %s order (competitive strategy): basisPrice = %d, bestPrice = %d, gapFactor = %v",
			sellStr(sell),
			basisPrice,
			bestPrice,
			gapFactor,
	...
	orders := func(orderPlacements []*OrderPlacement, sell bool) []*TradePlacement {
		...
		for i, p := range orderPlacements {
			...
			if sell && diffPercent < -0.05 {
				m.log.Tracef(
					"validate prepared %s order (competitive strategy): calculated rate %d is now lower than "+
			...
			if !sell && diffPercent > 0.05 {
				m.log.Tracef(
					"validate prepared %s order (competitive strategy): calculated rate %d is not higher than "+
						"start price %d for %v percent, not gonna place this order",
					sellStr(sell),
					rate,
					m.startPrice,
